class CLICommand:
    """Will compile the results from VASP calculations and place information into xlsx file.
    """

    @staticmethod
    def add_arguments(parser):
        parser.add_argument('adsorbates', nargs='*', help='The adsorbate you would like to examine. Write the names of the adsorbates after "PartD". If you want to examine all adsorbates, either write nothing or all')
...
def Run_PartD(args_partD):
    ...
    # Perform Part D method 
    adsorbates = args_partD.adsorbates

    if len(adsorbates) == 0:
        get_adsorbates = 'all'
    elif len(adsorbates) == 1 and adsorbates[0] == 'all':
        get_adsorbates = 'all'
    else:
        get_adsorbates = adsorbates
