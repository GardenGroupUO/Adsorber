class CLICommand:
    ...
    def add_arguments(parser):
        parser.add_argument('options', nargs='*', help='Options for comparing the structural binding of adsorbate to systems wither each other.')
...
def Run_method(args_compare):
	...
	def set_upper_energy_limit(input_value):
		...
		elif input_value.isdigit() or is_float(input_value):
	...
	args_compare_options = args_compare.options
	if len(args_compare_options) == 0:
		write_similarity_traj_files = set_write_similarity_traj_files(None)
		upper_energy_limit = set_upper_energy_limit(None)
	elif len(args_compare_options) == 1:
		write_similarity_traj_files = set_write_similarity_traj_files(args_compare_options[0])
		upper_energy_limit = set_upper_energy_limit(args_compare_options[0])
	elif len(args_compare_options) >= 2:
		write_similarity_traj_files = set_write_similarity_traj_files(args_compare_options[0])
		upper_energy_limit = set_upper_energy_limit(args_compare_options[1])
	# -----------------------------------------------
